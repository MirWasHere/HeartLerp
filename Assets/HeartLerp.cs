...
public class HeartLerp : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i =0; i < numSphere; i++){
            ...
            endPosition[i] = new Vector3(r * Mathf.Sqrt(2f) * Mathf.Pow(Mathf.Sin(i), 3),
            r * (-Mathf.Pow(Mathf.Cos(i), 3) - Mathf.Pow(Mathf.Cos(i), 2) + 2 * Mathf.Cos(i)) + 2,
            i / 5);
        }
    ...
    void Update()
    {
        ...
        for (int i =0; i < numSphere; i++){
            ...
            // startPosition[i]; 
            endPosition[i] = new Vector3(r * Mathf.Sqrt(2f) * Mathf.Pow(Mathf.Sin(i + time), 3),
            r * (-Mathf.Pow(Mathf.Cos(i + time), 3) - Mathf.Pow(Mathf.Cos(i + time), 2) + 2 * Mathf.Cos(i + time)) + 2,
            Mathf.Cos(lerpFraction * r - 15) * i / 2);
